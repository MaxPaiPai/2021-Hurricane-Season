# Validation for Hurricane Grace by Max Zhao
"""
...
def setrun(claw_pkg='geoclaw'):
    ...
    clawdata.lower[0] = -99.0      # west longitude 99
    ...
    amrdata.refinement_ratios_x = [2, 2, 3, 4]
    amrdata.refinement_ratios_y = [2, 2, 3, 4]
    amrdata.refinement_ratios_t = [2, 2, 3, 4]
    ...
    regions.append([3, 4, clawdata.t0, clawdata.tfinal, -98, -96, 25.5, 28.0]) # Gauge 1, 2, 3
    ...
    rundata.gaugedata.gauges.append([1, -97.03, 27.83,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])   # Aransas Pass, 8775241 
    rundata.gaugedata.gauges.append([2, -97.14, 26.07,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])   # SPI Brazos Santiago, 8779749
    rundata.gaugedata.gauges.append([3, -97.17, 26.07,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])   # South Padre Island CG Station, 8779748
...
def setgeo(rundata):
    ...
    atcf_path = os.path.join(scratch_dir, "bal072021.dat")
    ...
    grace = Storm(path=atcf_path, file_format="ATCF")

    # Calculate landfall time - Need to specify as the file above does not
    # include this info (8/26/2017 ~4am UTC)
    grace.time_offset = datetime.datetime(2021, 8, 20, 10)

    grace.write(data.storm_file, file_format='geoclaw')
