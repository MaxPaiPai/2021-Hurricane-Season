...
def setrun(claw_pkg='geoclaw'):
    ...
    clawdata.lower[0] = -99.0      # west longitude 90
    clawdata.upper[0] = -70.0      # east longitude 70

    clawdata.lower[1] = 8.0       # south latitude 18
    ...
    amrdata.refinement_ratios_x = [2, 2, 2, 2, 3, 4,]
    amrdata.refinement_ratios_y = [2, 2, 2, 2, 3, 4,]
    amrdata.refinement_ratios_t = [2, 2, 2, 2, 3, 4,]
    ...
    regions.append([3, 4, clawdata.t0, clawdata.tfinal, -99, -92, 25.0, 31.0]) 
   
    

    # regions.append([4, 5, clawdata.t0, clawdata.tfinal, -90.0, -89.0, 28.5, 29.5]) # Gauge 1
    # regions.append([4, 5, clawdata.t0, clawdata.tfinal, -90.5, -89.5, 29.0, 30.0]) # Gauge 2
    ...
    rundata.gaugedata.gauges.append([4, -94.71, 29.35,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])   # Galveston Bay Entrance, 8771341
    rundata.gaugedata.gauges.append([5, -97.14, 26.07,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])   # SPI Brazos Santiago, 8779749
    rundata.gaugedata.gauges.append([6, -93.83, 29.67,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])   # Texas Point, 8770822                               
...
def setgeo(rundata):
    ...
    atcf_path = os.path.join(scratch_dir, "bal142021.dat")
    ...
    nicholas = Storm(path=atcf_path, file_format="ATCF")

    # Calculate landfall time - Need to specify as the file above does not
    # include this info (8/26/2017 ~4am UTC)
    nicholas.time_offset = datetime.datetime(2021, 9, 14, 5, 30)

    nicholas.write(data.storm_file, file_format='geoclaw')
