...
def setplot(plotdata=None):
    ...
    import datetime
    my_dict = {1: '8727520', 2: '8726724', 3: '8726607', 4: '8726384', 5: '8725110', 6: '8724580'}
    def gauge_afteraxes(cd): # TO DO: how to plot, what is cd, where is data being read in?
        t0 = datetime.datetime(2021, 7, 5, 14)
        t_offset = datetime.datetime(2021, 7, 7, 14)
        tf = datetime.datetime(2021, 7, 8, 14)
        ...
        axes.set_xlim([-2, 1]) # change to t_0 and t_1
        axes.set_ylim([-1, 1.5])
        axes.set_xticks([ -2.0,-1.5, -1.0, -0.5, 0, 0.5, 1]) # adjust x_ticks appropriately
        axes.set_xticklabels([r"$-2.0$", r"$-1.5$", r"$-1.0$", r"$-0.5$", r"$-0$",r"$0.5$",r"$1.0$",])
        axes.set_yticks([-1, -.5, 0, .5, 1, 1.5])
        axes.set_yticklabels([r"$-1$", r"$-.5$", r"$0$", r"$.5$", r"$1$", r"$1.5$"])
        axes.grid(True)
    ...
    plotaxes.scaled = True
    plotaxes.xlimits = [-90, -80]
    plotaxes.ylimits = [23.0, 31.0]
