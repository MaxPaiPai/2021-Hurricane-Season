# Validation for Hurricane Ida by Max Zhao
"""
...
def setrun(claw_pkg='geoclaw'):
    ...
    clawdata.upper[0] = -78.0      # east longitude 78
    ...
    amrdata.refinement_ratios_x = [2, 2, 2, 3, 4, 4,]
    amrdata.refinement_ratios_y = [2, 2, 2, 3, 4, 4,]
    amrdata.refinement_ratios_t = [2, 2, 2, 3, 4, 4,]
    ...
    topo_file = topo_file.crop(( -94, -79, 22.5, 31.5))
    ...
    rundata.gaugedata.gauges.append([4, -89.30, 30.28,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])   # Bay Waveland Yacht Club, 8747437
    rundata.gaugedata.gauges.append([5, -90.21, 29.05,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])   # Port Fourchon, 8762075                    
...
def setgeo(rundata):
    ...
    atcf_path = os.path.join(scratch_dir, "bal092021.dat")
    ...
    ida = Storm(path=atcf_path, file_format="ATCF")

    # Calculate landfall time - Need to specify as the file above does not
    # include this info (8/26/2017 ~4am UTC)
    ida.time_offset = datetime.datetime(2021, 8, 29, 17)

    ida.write(data.storm_file, file_format='geoclaw')
